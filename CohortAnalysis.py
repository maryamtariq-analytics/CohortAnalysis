...
file_id = "1NUVXWw5hL6kQN4-7b26GJhPL-KN8AQVB"
...
st.write("We will make a new column indicating just the **month** and the **year** and will only pass 1 in place of "
         "day so that only month and year remain promenant in this column:")
# apply the function
...
st.write("Let's look at the dataset's tail so that we can get some sense for InvoiceMonth and CohortMonth:")
...
st.markdown("""
Following we'll create a pivot table that allows us to see customer retention (or other behaviors) over time, broken down by 
...
image_url = "https://raw.githubusercontent.com/maryamtariq-analytics/CohortAnalysis/main/CohortAnalysis1.png"
st.image(image_url)

st.write("**In Percentages:**")
# create percentage visual
plt.figure(figsize=(21,10))
sns.heatmap(percentage_cohort_table, annot=True, cmap='coolwarm', fmt='.0%')
